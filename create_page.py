...
class SudokuImageGenerator:
    def __init__(self):
        ...
        # Try to load font (you'll need to adjust the path)
        try:
            self.font = ImageFont.truetype("PlayfulTime-BLBB8.ttf", 30)
            self.title_font = ImageFont.truetype("PlayfulTime-BLBB8.ttf", 50)
        except:
    ...
    def draw_grid(self, draw, start_x, start_y):
        """Draw the sudoku grid"""
        for i in range(10):
            line_width = 3 if i % 3 == 0 else 1
            # Vertical lines
            draw.line([(start_x + i * self.cell_size, start_y),
                      (start_x + i * self.cell_size, start_y + self.grid_size)],
                     fill=self.grid_color, width=line_width)
            # Horizontal lines
            draw.line([(start_x, start_y + i * self.cell_size),
                      (start_x + self.grid_size, start_y + i * self.cell_size)],
                     fill=self.grid_color, width=line_width)

    def draw_numbers(self, draw, puzzle, start_x, start_y):
        """Draw the numbers in the grid"""
        for row in range(9):
            for col in range(9):
                number = puzzle[row][col]
                if number is not None:
                    text_bbox = draw.textbbox((0, 0), str(number), font=self.font)
                    text_width = text_bbox[2] - text_bbox[0]
                    text_height = text_bbox[3] - text_bbox[1]
                    x = start_x + col * self.cell_size + (self.cell_size - text_width) // 2
                    y = start_y + row * self.cell_size + (self.cell_size - text_height) // 2 - text_height//4
                    draw.text((x, y), str(number), fill="black", font=self.font)
...
def main():
    ...
    # Generate pages with two boards each
    counter_page = 1
    one_page = False
    for i in range(0, len(puzzles), 2):
        if i + 1 < len(puzzles):
            games = [puzzle['puzzle'] for puzzle in puzzles[i:i+2]]
            generator.generate_page(games, 'book/page_' + str(counter_page) + '.png', counter_page, puzzles[i]['difficulty_level'])
            counter_page += 1
            if one_page:
                break
        else:
